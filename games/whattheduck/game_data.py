...
class GameData:
    ...
    def game(self):
        ...
        # Constants
        SPEED_SMOOTHING = (
            0.8  # To smooth speed value and avoid jagginess, between 0 and 1
        )
        ...
        game_ui = pg_menu.Menu(
            "", self.console.size_x, self.console.size_y, theme=mytheme
        )
        time_label = game_ui.add.label(
            "Temps : 0.0s",
            font_color=self.console.WHITE,
            background_color=self.console.stone_background,
        )
        game_ui.add.vertical_margin(50)
        speed_label = game_ui.add.label(
            "Vitesse : 0.0m/s",
            font_color=self.console.WHITE,
            font_size=32,
            background_color=self.console.stone_background,
        ...
        game_ui.add.vertical_margin(50)
        distance_label = game_ui.add.label(
            "Distance : 0.0m",
            font_color=self.console.WHITE,
            background_color=self.console.stone_background,
        )
        ...
        while True:
            ...
            COEFF_DISTANCE
            speed = (
                SPEED_SMOOTHING * previous_speed
                + (1 - SPEED_SMOOTHING) * rot_speed_normalized
            )
            ...
            for event in events:
                if event.type == pg.QUIT or event.type == pg.MOUSEBUTTONDOWN:
                    self.console.display_score_ui(
                        time.time() - self.time0, 0, distance, 0
                    )
